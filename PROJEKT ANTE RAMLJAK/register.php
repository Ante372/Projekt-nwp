<?php 
	...
	if ($_POST['_action_'] == FALSE) {
		...
		<form action="" id="registration_form" name="registration_form" method="POST">
			...
			<label for="username">Username:* <small>(Korisnicko ime mora imati min 5 znakova a max 10)</small></label>
			<input type="text" id="username" name="username" pattern=".{5,10}" placeholder="Username.." required><br>
			
									
			<label for="password">Password:* <small>(Lozinka mora imati min 4 znaka)</small></label>
		...
		if ((int)$row['count'] == 0) {
			...
			echo '<p>Korisnik s ovim korisickim imenom vec postoji!</p>';
		}
	}
	print '
	</div>';
